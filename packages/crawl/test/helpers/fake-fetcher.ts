...
export type FakeRoute =
  string | { status: number; body?: string; finalUrl?: string } | { networkError: string }
 ...
 * which headers.
 */
export function fakeFetcher(routes: Record<string, FakeRoute>, log: FetchLogEntry[] = []): Fetcher {
  return (url, init) => {
